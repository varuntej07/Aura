...
gcloud run deploy "$SERVICE" \
  ...
  --memory=512Mi \
  --min-instances=0 \
  --set-env-vars="^@^GCP_PROJECT=${PROJECT}@OPS_PASSCODE=${OPS_PASSCODE}@POSTHOG_HOST=${POSTHOG_HOST:-https://us.i.posthog.com}@POSTHOG_PROJECT_ID=${POSTHOG_PROJECT_ID:-}@POSTHOG_PERSONAL_KEY=${POSTHOG_PERSONAL_KEY:-}@OPS_UTC_OFFSET_HOURS=${OPS_UTC_OFFSET_HOURS:-0}@OPS_POSTHOG_WEB_PROJECT_ID=${OPS_POSTHOG_WEB_PROJECT_ID:-}@GITHUB_TOKEN=${GITHUB_TOKEN:-}@OPS_CRASHLYTICS_BQ_DATASET=${OPS_CRASHLYTICS_BQ_DATASET:-firebase_crashlytics}@OPS_GCP_BILLING_TABLE=${OPS_GCP_BILLING_TABLE:-}@OPS_BRAVE_COST_PER_QUERY_USD=${OPS_BRAVE_COST_PER_QUERY_USD:-}@OPS_PROVIDER_MONTHLY_COSTS_JSON=${OPS_PROVIDER_MONTHLY_COSTS_JSON:-}"
