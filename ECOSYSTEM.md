...
- **PostHog**: the app side (Aura mobile, Aura-Desktop, and `juno-backend`'s own server-side capture) confirmed share one project; funnel event names are contract-tested (`backend/src/services/analytics/funnel_events.py` vs `lib/core/analytics/funnel_events.dart`).
  - **Platform identification.** Aura-Desktop stamps `platform: "desktop-react"` on every event and relies on `$os` (`Windows` / `macOS`) for the OS split. Mobile sets no `platform` property at all and is identified by `$os` (`Android` / `iOS`). Any consumer filtering desktop on `platform = "windows"` matches nothing, and one filtering on `$os = "Windows"` alone silently drops every macOS user.
  - **Latency event names differ per client and are NOT interchangeable.** Mobile emits `voice_start_to_first_talk` with `elapsed_ms` (voice start to first talk). Aura-Desktop emits `voice_first_response` with `tapToFirstResponseMs`, measured from the user's KEYPRESS, so it also spans summon, token mint, connect and agent join. They measure different windows; do not "unify" one into the other without changing what the number means.
  - **Aura-Desktop runs `posthog-js` since 0.15.8** (`dist/module.full.no-external`, recorder bundled because the desktop CSP forbids external scripts; the SDK still fetches its remote config as JSON from `us-assets.i.posthog.com`, which the CSP allows). Capture starts opted out and is switched on by the same consent flag that gates Sentry, in every window. Session replay is on with `maskAllInputs`, `maskTextSelector: "*"` and `ph-no-capture` on every chat, draft, meeting, history, interview and dictation surface, and is disabled outright in the dictation HUD and status-pill windows. The full desktop event list is the `EVENT_DESKTOP_CLIENT_*` block in `funnel_events.py` (mirrored in `src/lib/analyticsEvents.ts` as a TypeScript union). `desktop_heartbeat` fires every 10 minutes while the app runs; `client_log` carries rate-limited, redacted WARN/ERROR lines, the same shape as the Flutter `app_logger` capture.
  - **`chat_e2e_latency` is a shared contract.** Both `lib/presentation/viewmodels/chat_viewmodel.dart` and `Aura-Desktop/src/overlay/useChatSession.ts` emit it with the same four properties (`ttft_ms`, `total_ms`, `agent_type`, `outcome`), from every terminal path rather than success only, so one ops query serves both. `ttft_ms` is the first non-empty visible text delta on both sides: not a thinking delta, tool, or status frame. Renaming a property on one side blanks that platform's latency panel. Aura-Web initializes its own `posthog-js` client from its own env vars (`NEXT_PUBLIC_POSTHOG_KEY`); whether that resolves to the same PostHog project as the app side has not been verified, see "Known gaps."
...
- **Sentry (desktop crash reporting)**: Aura-Desktop reports native Rust panics (`sentry` crate, `src-tauri/src/sentry_setup.rs`) and webview JS errors (`@sentry/browser`, `src/lib/sentry.ts`) into ONE Sentry project (org `o4511685555519488`, project `4511685630361600`).
  ...
  Since 0.15.8 the same project also receives native crash minidumps (SIGSEGV, stack overflow, Objective-C exceptions) from a crash-reporter child process (`sentry-rust-minidump`, `src-tauri/src/telemetry.rs`): the desktop binary relaunched with `--crash-reporter-server`. A dump taken while a dictation hold was in flight is dropped by a fail-closed marker-file gate in `sentry_setup.rs`, because it could carry transcript bytes; the crash is still counted locally for the crash-loop beacon. Every JS window reports with `window_label`, `app_version` and `os` tags and the Firebase uid as the Sentry user.
  This repo's ops dashboard does NOT read that project. It once had a `sentry_provider.py`, but nothing ever imported it and `ops/deploy.sh` never passed the credentials, so it was deleted on 2026-09-07; the Desktop tab points at Cloud Logging for operational errors instead. A Sentry project change on the desktop side therefore has no ops-side mirror to update. Re-adding a reader is a new integration, not a repair.
- **Langfuse (LLM observability)**: `juno-backend` and the LiveKit voice worker each write one generation per LLM provider attempt and one span per tool call (`backend/src/services/analytics/llm_telemetry.py`, metadata and token usage only, never prompt text).
  ...
  **The worker's copy of those three is a separate credential set that lives in LiveKit agent secrets, not in Cloud Run.** `llm_telemetry` degrades to a silent no-op when they are absent, so a worker deployed without them emits NOTHING and the gap is invisible from the Langfuse side except as an absence. That was the state until 2026-09-09: the deployed agent had 12 secrets and none of them Langfuse, so no production voice session had ever been traced, while local test runs wrote fixture-uid traces (`u1`, `uid-1`, `uid-voice-7`) that made the project look populated. Check with `lk agent secrets list --id <agent-id>`, and confirm on a live session that `llm_telemetry: no LANGFUSE keys, LLM observability disabled` is absent. Setting or rotating them on the backend does not propagate to the worker; it needs its own `lk agent secrets set` plus a redeploy.
  Independently of Langfuse, LiveKit Cloud ships the worker's stdout to Cloud Logging at `projects/juno-2ea45/logs/cloud-agents`, which is the only place past voice sessions can be read: `lk agent logs` tails live output only and has no history flag. The `LOGS_ENABLE_GCP` worker secret is vestigial and referenced nowhere in this repo; that shipping is LiveKit-side.
...
blocked throughout.
`GET /meetings/recent` returns the note without transcript turns for a bounded dashboard payload. `GET /meetings/{id}` returns the full note with transcript. Both use an explicit public note-field allowlist and project the immutable canonical pointer as `transcript_artifact`. A transcript turn is `{speaker, text}` plus, from `meeting-transcript-v3` on, meeting-relative `start_s`/`end_s`. Those two are additive and OPTIONAL: notes published before v3 carry neither, and Aura-Desktop renders such a turn without a timestamp rather than rejecting the transcript.
...
|---|---|---|
| `POST /diagnostics/desktop` | UNAUTHENTICATED crash-loop beacon, posted from the main window only when Rust found the previous run did not exit cleanly, and only after telemetry consent. Body allowlist: `install_id`, `app_version`, `os_platform` (`windows`/`macos`), `os_version`, `os_arch`, `launch_count`, `consecutive_failed_launches`, `last_exit{kind: clean/crash/unknown, at}`, optional `crash_reporter_alive`, `previous_app_version`. 5 s client timeout; a 404 is treated as "not deployed yet" and never retried. | Always `{ok: true}` once well formed; stored in Firestore `startup_diagnostics` with `platform: "desktop"`, 30-day TTL, 20 reports per install per hour. |
...
When Desktop receives `reauthorization_required`, it calls authenticated `POST /connectors/oauth/authorize` with an allowlisted `google_calendar`, `gmail`, or `notion` connector. juno-backend creates an owner-bound, ten-minute `connector_oauth_attempts` row with a PKCE verifier and returns the provider's authorization URL (Notion does not use PKCE; the verifier is stored but never sent). The provider calls `GET /connectors/oauth/google/callback` or `GET /connectors/oauth/notion/callback` on juno-backend, which atomically claims the attempt, exchanges the code, and writes the existing authoritative integration document. The callback opens `aura://connectors/complete`; Desktop consumes the result and refreshes `GET /connectors` once. There is no web dashboard, browser custom-token exchange, or connector polling loop.
...
A destination can also be bound AFTER the fact, for a run that finished without one: `POST /research/{run_id}/deliver` takes `{data_source_id, database_name}`, writes `delivery`, and enqueues the same `notion_deliver` stage, so there is no second stage, notification type, or client contract. It refuses with HTTP 400 and a named reason when the run is still running (`still_running`), already carries a destination (`already_bound`), has no brief worth writing (`not_deliverable`), or is gone (`not_found`); the first three come from the same `may_run_post_terminal` gate the stage itself is admitted through. Binding at creation stays immutable: this route only fills an ABSENT `delivery`, so re-routing a bound run is still a new run and never an edit. The voice worker reaches it through `deliver_research_to_notion`, which resolves the run from session state rather than from the model. Aura-Desktop needs no change, since the Research page already renders `delivery`/`delivery_result` from the run projection.

The capture itself is voice-only and desktop-only. The LiveKit worker's `save_to_notion` tool extracts a typed record worker-side (screen bytes never leave worker RAM except into one tool-free model call) and calls juno-backend `POST /notion/resolve`, `POST /notion/write`, `POST /notion/undo`, and `POST /notion/create-database`, authenticated with the session's per-user Firebase ID token (the same credential as `/mcp`, deliberately NOT under `/internal/*`). Write receipts live at `UserAura/{uid}/notion_writes/{idempotency_key}` (durable failure records in the sibling `notion_write_failures` subcollection, same key); account deletion already enumerates the `UserAura` root. After a durable write the worker publishes the data-channel event `notion.saved` with `{database_name, page_url, page_id}`; Aura-Desktop validates it in `agentData.ts` and shows a caption. Old desktop builds ignore the unknown event type safely (it is a caption, not an actionable notification, so the outbox type-drop hazard from contract 5d does not apply).

Added 2026-09-04, screen-context availability signalling (voice data channel, both directions, version-skew safe in both):

- Desktop -> worker `screen_context.unavailable` `{reason}` on topic `client_events`, published at most once per spoken turn when the client SKIPS per-turn capture. `reason` is a closed vocabulary (`screen_context_disabled`, `permission_denied`, `mode_conflict`, `signed_out`, `capture_failed`); the worker normalizes anything else to `capture_failed` (`agent/voice/screen_context_control.py`) and uses the freshest signal to pick a specific spoken line when a save finds no screen. An old worker ignores the unknown control type.
- Worker -> desktop `screen_context.request` (empty payload), published by the `enable_screen_context` voice tool. The desktop is the sole authority over the `voiceScreenContext` privacy setting: it renders a consent card in the overlay and only the user's own click enables anything. An old desktop drops the unknown event type safely. The desktop also mirrors the setting into native authorization state via the `set_voice_screen_context` Tauri command (internal, not cross-repo).
