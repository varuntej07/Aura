...
**Per-tool voice skills:** keep the permanent persona prompt lean. A local voice tool with nontrivial selection or output rules must register a `skill_name` in `voice/capabilities.py` and define its focused brief in `voice/tool_skills.py`; `voice/action_policy.py` injects only the briefs for capabilities detected from the finalized turn. This includes a write skill while its tool is withheld for a policy-owned clarification. Skills guide model behavior but never authorize it. Deterministic surface, finalized-turn, fresh-frame, required-field, dependency, and side-effect checks remain the authority. Add registry, policy, execution, and prompt-injection tests whenever a tool or skill is added.
...
**Unresolved voice actions:** state belongs to the finalized source message and source turn. It is valid only for the immediately following user turn, and only when that utterance is a plausible answer to the owned missing slot. A correction, unrelated classified capability, or any completed tool clears it. Missing slots merge only while the owning capability remains active. Session summaries are informational and cannot create or extend unresolved state.
