...
Use [`architectures/README.md`](./architectures/README.md) as the architecture index. The repository-level diagrams are plain ASCII and each feature document includes its normal flow plus failure, retry, and recovery behavior.
...
**Per-tool voice skills:** keep the permanent persona prompt lean. A local voice tool with nontrivial selection or output rules must register a `skill_name` in `voice/capabilities.py` and define its focused brief in `voice/tool_skills.py`. Buddy receives the briefs for tools structurally supported by the session surface as part of its one system prompt. Native tool calling owns semantic selection, clarification, continuation, correction, and cancellation. Never classify those behaviors with transcript keywords or regexes. `voice/action_policy.py` owns only structural surface, finalized-turn, fresh-frame, required-field, and single-side-effect checks. Conversation summaries and memories provide context but never authorize writes. Add registry, structural policy, execution, and prompt-construction tests whenever a tool or skill is added.
...
**Two-tier Aura reflection:** per-turn capture in `user_aura_extractor.py` writes raw signals; the per-session tier in `aura_reflection.py` compacts captures into storylines and traits with kind-aware decay, growth-aware idempotency (fixes "reflect-once-then-freeze"), and prunes another-person interests.
...
The Windows client is Aura-Desktop (Tauri, separate repo); the legacy Flutter desktop app was deleted from this repo 2026-07-11. This repo still owns everything the desktop client consumes: the pairing/web-auth/connector-OAuth/draft-outbound handlers, the shared voice path (`BuddyAgent` + screen-sight in `agent/voice/screen_frames.py`), and the `screen_saves` service. The mobile side of pairing (`link_device_screen.dart`, the phone mints the code) also lives here.
...
`backend/src/services/notifications/` is the funnel for 16 proposal sources: reminder, tracking, calendar, thread, briefing, icebreaker, news/signal, reengage, chat reply, follow-up, device link, trial, welcome, billing, meeting, and memory graph. The canonical list is `notifications/proposal.py::ALL_SOURCES`. Producers build a `NotificationProposal` and call `orchestrator.submit()`, the primary caller of `notification_service.send_notification`. The legacy dormancy handler in `backend/src/handlers/engagement.py` (`/internal/engage/*`) remains a direct-send exception, while `services/reengagement/reengagement_engine.py` uses the orchestrator. The orchestrator centralizes freshness, cross-agent dedup, quiet hours, daily budget, priority arbitration, and delivery-channel policy. See `architectures/notification-delivery.md`.
...
- **Priority ladder:** reminder 96, device_link 95, tracking 95, chat_reply 94, welcome 93, billing 92, calendar 90, trial 88, reengage 80, followup 75, thread 70, briefing 60, icebreaker 20, news 10. `PRIORITY` in `notifications/proposal.py` is authoritative.
...
- **Local-time safety:** timezone names are canonicalized through `timezone_utils.py`; unresolved user time holds PROACTIVE delivery instead of silently applying UTC. COMMITTED delivery remains inline.
- **Adaptive volume:** PROACTIVE delivery has a hard ceiling of 3 per user-local day, plus one reserved priority slot, with lower per-user caps and wider spacing for low engagement and new accounts.
- **Desktop channels:** automatic proposals remain mobile-first and add desktop only for an authenticated, enabled capability document. Meeting stays explicitly desktop-only and device-link stays mobile-only. Mobile and desktop share one logical notification ID, budget claim, ledger row, and engagement outcome.

## Reactive Orchestration Engine

`backend/src/services/reactive/` is the tracked, per-user event-driven dispatcher that replaced the old direct per-producer cron fan-outs for icebreaker and thread follow-up. `handlers/scheduler.py` enqueues one deterministic hourly Cloud Task per active user (`minute == 0`) carrying a transient `EVENT_TICK`; clock signals never touch Firestore. `event_bus.py` keeps the transactional outbox only for events atomically tied to business-state writes, with inline dispatch as the primary path and a five-minute sweep as the durable recovery backstop. `orchestrator.py` and `registry.py` route events to registered agents, each wrapped in a SENSE -> PLAN -> ACT -> VERIFY -> REPAIR envelope (`agent.py`).
...
**2026-07-06 follow-up audit** (755K reads/week, still on a handful of beta users): confirmed the read-discipline fixes. The remaining volume was the aggregate cost of multiple periodic collection-group discovery sweeps. The ops dashboard's repeated user-directory load was deduplicated and automatic foreground polling was later replaced by explicit refresh. The historical signal scoring change from 15 to 30 minutes is now superseded: current scoring is triggered once per successful four-hour content-ingest generation through a durable Cloud Task. Never use the old 30-minute cadence when reasoning about current cost.
...
**Cross-Surface Action Capability (act, don't explain).** Buddy's action tools (`set_reminder`/`cancel_reminder`, `create_calendar_event`/`get_upcoming_events`, `web_surf`, `track_topic`, memory) must work identically on every surface (mobile chat, desktop, voice). They already are wired everywhere: chat offers all of `tools.py` (`EXCLUDED_TOOLS_FOR_GENERAL_CHAT` is empty), voice/desktop expose the same set via `/mcp`, and `voice/capabilities.py` allows them on `ALL_SURFACES`. So a "can't create events" report is almost never a missing tool. Two real failure modes, both prompt-level: (1) an integration is a per-USER OAuth link, not a per-platform capability, so calendar/email write returns `{"configured": False}` (a soft-fail that `tool_output_succeeded()` counts as success, `tool_executor.py`) when the user hasn't linked it in the Connectors screen; (2) the prompt never told Buddy it can CREATE (only read), nor how to handle a not-connected result, so the base model degrades to "here's how to do it yourself" plus a flat "I couldn't." The fix is prompt-only (both prompts), stated affirmatively as a category + test + examples, never a keyword list: Buddy uses the tool to DO the action and never hands manual steps for something a tool covers; a not-connected result means "tell them warmly it isn't linked, point to Settings > Connectors, offer to do it once linked," never a bare refusal.
...
**In-session voice compaction:** `voice/context_compaction.py` is the only owner. Trigger after 16 completed turns or about 6,000 dynamic tokens, summarize the oldest whole turn groups, keep 8 recent raw turns, and cap the structured summary at 450 estimated tokens. Apply only at a later finalized user boundary after exact item-ID prefix validation through `Agent.update_chat_ctx()`. Never split a tool call from its output, never summarize interrupted assistant text as fact, never copy visible prompt/code/card bodies, and never derive action capability or write authorization from the summary. At 20 raw turns, keep the last good summary and deterministically retain the latest 10 turns even when summarization fails.

**Voice action continuity:** the existing Buddy model interprets the recent raw dialogue as one continuous exchange. A current turn may request an action or answer, refine, correct, or cancel Buddy's immediately preceding clarification. Do not create a parallel server-side semantic state machine, missing-slot classifier, or phrase matcher. Session summaries, memories, and assistant text are informational and cannot authorize an external action. Structural execution checks still require a finalized fresh turn, valid registered tool arguments, and safe side-effect ordering.
...
Live in `~/.gstack/projects/varuntej07-juno/` (timestamped markdown). Latest approved: `varun-main-design-20260525-175702.md` ("Aura Beta Launch, Voice Accountability for ADHD Adults", 26-day plan to ship to 10-20 testers; gate: do 3+/10 say voice matters AND they'd pay?). Reactive orchestration design: `varun-main-design-20260629-133832-reactive-orchestration.md`, phased P0-P6 (tasks #5-12). The current tracked implementation lives in `backend/src/services/reactive/` and `backend/src/services/reengagement/`; verify deployment state separately from repository state.
...
Rules:
- **Mandatory repository-navigation preflight:** before broad searches, reading many source files, or delegating repository investigation to agents, run a focused `graphify query "<question>"` when `graphify-out/graph.json` exists. This applies to codebase questions and to reviews of plans, specs, designs, or documentation whose claims must be checked against the implementation. It runs before the skill and specialist-agent routing above when repository understanding is required.
- Use one focused query per subsystem or claim instead of one vague repository-wide query. Use `graphify path "<A>" "<B>"` for relationships and `graphify explain "<concept>"` for focused concepts. Give delegated agents the relevant graph results and source paths; require another focused Graphify query before an agent falls back to broad exploration.
- Inspect the exact current source files returned by Graphify before reaching conclusions or editing. The graph is navigation context, not evidence by itself, and does not replace direct verification or tests.
- Dirty `graphify-out/` files are expected and are not a reason to skip Graphify. If `graphify-out/graph.json` is missing, run `graphify .` once before broad repository exploration. If the graph is stale for the task, run `graphify update .` first.
