...
`backend/src/services/reactive/` is the tracked, per-user event-driven dispatcher that replaced the old direct per-producer cron fan-outs for icebreaker and thread follow-up. `handlers/scheduler.py` emits an hourly `EVENT_TICK` per active user (`minute == 0`) onto `event_bus.py`; the every-minute outbox sweep provides durable relay recovery. `orchestrator.py` and `registry.py` route coalesced events to registered agents, each wrapped in a SENSE -> PLAN -> ACT -> VERIFY -> REPAIR envelope (`agent.py`).
