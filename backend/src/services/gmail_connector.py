...
via the shared GoogleConnectorBase. Credentials live at
users/{uid}/integrations/gmail. Reads are on-demand (no webhook sync) and
surfaced to Buddy through the list_emails / read_email / send_email tools.
"""
...
class GmailReauthorizationRequired(ReauthorizationRequired):
...
class GmailConnector(GoogleConnectorBase):
    ...
    API_NAME = "gmail"
    API_VERSION = "v1"
    NOT_CONNECTED_ERROR = "Gmail is not connected."
    EXPIRED_ERROR = "Gmail connection has expired. Reconnect required."
    REAUTH_MESSAGE = "Gmail authorization is required."
    ...
    def _persist_credentials(
        ...
        *,
        ...
        super()._persist_credentials(
            access_token=access_token,
            refresh_token=refresh_token,
            expiry_at=expiry_at,
            enabled=enabled,
            last_error=last_error,
            extra_fields={"email_address": email_address} if email_address else None,
        )
