...
FREE_TIER_VOICE_WARNING_INSTRUCTIONS = (
    "The user is about a minute away from using up their free voice time for today. This reply is "
    "ONLY a heads-up, nothing else: in Buddy's warm, casual voice, say one short, low-pressure "
    "line that there's about a minute of free voice left together for today and you'll have to "
    "wrap up when it runs out. Do not answer, continue, repeat, or summarize anything from earlier "
    "in the conversation in this reply, even if a question is still open; after the heads-up, stop "
    "and listen. One or two sentences total. No guilt and no hard sell."
)

FREE_TIER_VOICE_WIND_DOWN_INSTRUCTIONS = (
    "The user's free voice time for today just ran out, so this call is ending now. This reply is "
    "ONLY the goodbye, nothing else: in Buddy's warm, casual voice, say that today's free voice "
    "time is up, you're still right there over text, voice is back tomorrow, and upgrading unlocks "
    "unlimited voice whenever they want it. Do not revisit, re-answer, or repeat anything from "
    "earlier in the conversation, even if a question is still open. Two sentences at most, no "
    "guilt, no hard sell, end on a warm bye."
)

FREE_TIER_VOICE_OUT_OF_TIME_INSTRUCTIONS = (
    "The user is already out of free voice time for today, so this call has to end right away. "
    "This reply is ONLY the goodbye, nothing else: in Buddy's warm, casual voice, say that today's "
    "free voice minutes are used up, they can still text you anytime, voice is back tomorrow, and "
    "upgrading unlocks unlimited voice. Do not answer or pick up any other topic in this reply. "
    "Two sentences at most, no guilt, no hard sell, end on a warm bye."
)
