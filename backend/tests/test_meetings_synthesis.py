...
must survive into the saved note, and raw audio remains recoverable until an
explicit deletion saga targets its exact generations.
...
class _Env:
    ...
    def __init__(
        ...
        *,
        ...
        self.gcs_paths = [
            f"meetings/{UID}/{MID}/{int(seg['seq']):04d}.flac" for seg in (segments or [])
        ]
        ...
        monkeypatch.setattr(synthesis.openai_stt, "transcribe_segment", fake_transcribe)
    ...
    def _set_llm(self, monkeypatch, *, ok: bool):
        ...
        class _Provider:
            async def balanced(self, prompt, *, system=None, response_model=None, temperature=0.5):
                if not ok:
...
def test_segments_past_cap_are_dropped(monkeypatch):
    env = _Env(
        monkeypatch, cap_minutes=60, segments=[_seg(0, 0), _seg(11, 55), _seg(12, 60), _seg(13, 65)]
    )
    status = asyncio.run(synthesis.run_synthesis(UID, MID))
    assert status == F.STATUS_READY
...
def test_primary_stt_infra_failure_uses_deepgram_and_keeps_audio(monkeypatch):
    env = _Env(monkeypatch, segments=[_seg(0, 0)])

    async def boom(data):
        raise dg.DeepgramError("deepgram down")

    monkeypatch.setattr(synthesis.openai_stt, "transcribe_segment", boom)
    status = asyncio.run(synthesis.run_synthesis(UID, MID))
    assert status == F.STATUS_READY
    assert not env.audio_deleted


def test_both_stt_providers_reject_is_terminal_not_a_retry_storm(monkeypatch):
    """When both providers reject the bytes, the run must settle visibly
    instead of retrying the identical audio forever."""
    env = _Env(monkeypatch, segments=[_seg(0, 0)])

    async def primary_rejected(data):
        raise dg.DeepgramRejectedError("400 rejected audio")

    async def rejected(data):
        raise dg.DeepgramRejectedError("400 corrupt flac")

    monkeypatch.setattr(synthesis.openai_stt, "transcribe_segment", primary_rejected)
...
def test_forged_offsets_cannot_stretch_the_cap(monkeypatch):
    ...
    env = _Env(monkeypatch, cap_minutes=60, segments=[_seg(i, 0) for i in range(40)])
    status = asyncio.run(synthesis.run_synthesis(UID, MID))
    assert status == F.STATUS_READY
...
def test_ready_path_retains_audio(monkeypatch):
    env = _Env(monkeypatch, segments=[_seg(0, 0)])
    status = asyncio.run(synthesis.run_synthesis(UID, MID))
    assert status == F.STATUS_READY
    assert not env.audio_deleted
